...
class FaustScapeDataset(Dataset):
    def __init__(self, root_dir, name="faust", train=True, k_eig=128, n_fmap=30, use_cache=True, op_cache_dir=None):
        ...
        for iFile in range(len(mesh_files)):
            ...
            #mesh = pp3d.read_mesh(mesh_files[iFile])
            ...
            self.heat_tensors.append(torch.from_numpy(heat_fs))
            self.wave_tensors.append(torch.from_numpy(wave_fs))
            ##############
        ...
        for i in range(len(self.hks_list)):
            if len(self.verts_list[i]) != len(self.heat_tensors[i]):
                print("There are {} verts but {} heat tensors on {}".format(len(self.verts_list[i]), len(self.heat_tensors[i]), i))

        # self.mma_list = [torch.cat([self.verts_list[i],self.heat_tensors[i],self.wave_tensors[i]], dim=-1) for i in range(len(self.verts_list))]
